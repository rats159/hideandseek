...
measure_text :: proc "c" (text: clay.StringSlice, config: ^clay.TextElementConfig, userData: rawptr) -> clay.Dimensions {
    ...
    font := ([^]rl.Font)(userData)[config.fontId]
    ...
    return {width = line_width * scaleFactor, height = f32(config.fontSize)}
}

clay_raylib_render :: proc(render_commands: ^clay.ClayArray(clay.RenderCommand), fonts: [dynamic]rl.Font, allocator := context.temp_allocator) {
